...
export function FontDetailDialog() {
  ...
  return (
    <Dialog.Root open={Boolean(id)} onOpenChange={(o: boolean) => !o && close()}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm" />
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[92vh] w-[92vw] max-w-[420px] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-lg border bg-popover p-4 text-popover-foreground shadow-2xl">
          <Dialog.Title className="sr-only">Font details</Dialog.Title>
          <Dialog.Description className="sr-only">Preview and edit a saved font.</Dialog.Description>
          {id && <Body id={id} onClose={close} />}
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  );
...
function Body({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <div style={{ fontFamily: stack }} className="truncate text-2xl font-semibold">
        {font.family}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={() => { void deleteFont(font.id); onClose(); }}

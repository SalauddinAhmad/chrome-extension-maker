...
import { ColorLibrary } from "./ui/color-library";
import { GradientLab } from "./ui/gradient-lab";
import { PalettePanel } from "./ui/palette-panel";
import { BrandBuilder } from "./ui/brand-builder";
    ...
    <div className="space-y-3 p-3">
      <header className="flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <div className="text-[10px] text-muted-foreground">Pick · library · palette · brand · contrast · export</div>
        </div>
      </header>

      <div className="grid grid-cols-7 gap-1 rounded-md border bg-muted/40 p-1">
        {TABS.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
      ...
      {tab === "library" && <ColorLibrary />}
      {tab === "palette" && <PalettePanel />}
      {tab === "brand" && <BrandBuilder />}

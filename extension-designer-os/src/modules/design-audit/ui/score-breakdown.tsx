import { AUDIT_CATEGORY_LABEL, type AuditScore } from "@/types";
...
export function ScoreBreakdown({ scores }: { scores: AuditScore[] }) {
  return (
    <div className="space-y-2">
      {scores.map((s) => (
        <div key={s.category} className="rounded-md border bg-card p-2">
          <div className="flex items-center justify-between text-xs">
            <div className="font-medium">{AUDIT_CATEGORY_LABEL[s.category]}</div>
